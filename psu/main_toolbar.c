/**
 * PSU toolbar GUI — minimal strip: big V/A per channel, status, SET popup.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* Compact strip; window grows when set-popup is open */
#define WIN_W           900
...
/* Popup sits below the strip — never centered over live V/A */
#define POPUP_PW           300
#define POPUP_PH           204
...
typedef struct {
    ...
    TTF_Font *font_label;   /* small channel / units */
    TTF_Font *font_num;     /* big V/A */
    TTF_Font *font_stat;    /* ON/OFF CV/CC */
    TTF_Font *font_pop;     /* popup body */
    psu_context_t psu;
    psu_status_t ch[2];
    bool running;
    bool demo_mode;
...
static void draw_header(void) {
    ...
    fill_rect(g_app.renderer, 0, 0, WIN_W, HEADER_H);
    set_color(g_app.renderer, COL_BORDER);
    SDL_RenderDrawLine(g_app.renderer, 0, HEADER_H - 1, WIN_W, HEADER_H - 1);
    ...
    draw_led(g_app.renderer, WIN_W - 12, HEADER_H / 2, 4, true, dot);
}

/**
 * One channel column: CH label | big V | big A | status | SET | OUT
 * @param register_hits  if false, draw only (e.g. modal open — popup owns clicks)
 */
static void draw_channel_column(int idx, int x, int y, int w, int h, bool register_hits) {
    psu_status_t *st = &g_app.ch[idx];
    ...
    draw_text(g_app.renderer, g_app.font_label, chname, x + 6, y + 4, COL_DIM, 0);
    ...
    /* Status strip (top-right), separate from V/A line to avoid overlap */
    int stat_x = x + w - 196;
    ...
    if (register_hits) {
        draw_btn(set_x, btn_y, 40, 24, "SET", set_id, false);
        draw_btn(out_x, btn_y, 36, 24, "OUT", out_id, st->out_on);
    } else {
...
static void popup_open_for(int ch_idx) {
    g_app.popup_open = true;
    g_app.popup_ch = ch_idx;
    g_app.popup_focus = 0;
    snprintf(g_app.popup_v, sizeof(g_app.popup_v), "%.2f", g_app.ch[ch_idx].set_v / 100.0f);
    snprintf(g_app.popup_a, sizeof(g_app.popup_a), "%.3f", g_app.ch[ch_idx].set_a / 1000.0f);
    SDL_SetWindowSize(g_app.window, WIN_W, WIN_H_POPUP);
}

static void popup_apply(void) {
    int i = g_app.popup_ch;
    float fv = (float)atof(g_app.popup_v);
    float fa = (float)atof(g_app.popup_a);
    int ch = i + 1;

    if (fv >= 0.0f && fv <= 36.0f) {
        if (!g_app.demo_mode) psu_set_voltage(&g_app.psu, ch, fv);
        else g_app.ch[i].set_v = (uint16_t)(fv * 100.0f);
    }
    if (fa >= 0.0f && fa <= 6.0f) {
        if (!g_app.demo_mode) psu_set_current(&g_app.psu, ch, fa);
        else g_app.ch[i].set_a = (uint16_t)(fa * 1000.0f);
    }
    popup_close();
}
...
static void draw_popup(void) {
    ...
    /* Dim only below the live strip — toolbar stays clear */
    ...
    char title[24];
    snprintf(title, sizeof(title), "SET CH%d", g_app.popup_ch + 1);
    draw_text(g_app.renderer, g_app.font_pop, title, px + 14, py + 12, COL_INPUT_FG, 0);
...
static void render(void) {
    ...
    draw_channel_column(0, gap, y0, col_w, STRIP_BLOCK_H, hits);
    draw_channel_column(1, gap * 2 + col_w, y0, col_w, STRIP_BLOCK_H, hits);
...
static void handle_click(int mx, int my) {
    ...
    switch (btn) {
        case BTN_CH1_SET: popup_open_for(0); break;
        case BTN_CH2_SET: popup_open_for(1); break;
        case BTN_CH1_OUT: {
            bool en = !g_app.ch[0].out_on;
            if (!g_app.demo_mode) psu_set_output(&g_app.psu, 1, en);
            else g_app.ch[0].out_on = en ? 1 : 0;
            break;
        }
        case BTN_CH2_OUT: {
            bool en = !g_app.ch[1].out_on;
            if (!g_app.demo_mode) psu_set_output(&g_app.psu, 2, en);
            else g_app.ch[1].out_on = en ? 1 : 0;
            break;
        }
        default: break;
    }
...
static void update_demo(void) {
    ...
    phase += 0.08f;
    for (int i = 0; i < 2; i++) {
        psu_status_t *st = &g_app.ch[i];
        st->set_v = 1200 + i * 300;
        st->set_a = 1500 + i * 400;
        st->out_v = st->set_v - (uint16_t)(20.0f + 10.0f * SDL_sinf(phase + i));
        st->out_a = st->set_a - (uint16_t)(30.0f + 15.0f * SDL_sinf(phase * 0.7f + i));
